...
- `Subagent (<role>):` template → OpenCode's `task` tool with `subagent_type` by role: implementation/coding → `"fixer"`, code/spec/plan review → `"oracle"`, codebase exploration/search → `"explorer"`, library/doc research → `"librarian"`, UI/UX → `"designer"`
